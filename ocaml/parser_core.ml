...
let max_trees_per_chart_cell = 100

let rec list_length_at_least n xs =
  if n <= 0 then
    true
  else
    match xs with
    | [] -> false
    | _ :: rest -> list_length_at_least (n - 1) rest
...
let rec add_tree chart unary_rules nt i j tree =
  ...
  if List.mem tree existing then
    ()
  else if list_length_at_least max_trees_per_chart_cell existing then
    ()
  else begin
    Hashtbl.replace chart (nt, i, j) (tree :: existing);
...
let split_chart_rules rules =
  List.fold_left
    (fun (terminal_rules, unary_rules, binary_rules) rule ->
       match rule with
       | TRule (lhs, terminal) ->
           ((lhs, terminal) :: terminal_rules, unary_rules, binary_rules)

       | NTRule (lhs, [child]) ->
           (terminal_rules, (lhs, child) :: unary_rules, binary_rules)

       | NTRule (lhs, [left; right]) ->
           (terminal_rules, unary_rules, (lhs, left, right) :: binary_rules)

       | _ ->
           (terminal_rules, unary_rules, binary_rules))
    ([], [], [])
    rules

let build_terminal_index terminal_rules =
  let table = Hashtbl.create 4096 in

  List.iter
    (fun (lhs, terminal) ->
       let old =
         match Hashtbl.find_opt table terminal with
         | Some cats -> cats
         | None -> []
       in
       if List.mem lhs old then
         ()
       else
         Hashtbl.replace table terminal (lhs :: old))
    terminal_rules;
...
let parse_trees_chart_from cfg start_symbol input =
  ...
  let terminal_rules, unary_rules, binary_rules =
    split_chart_rules rules
  in

  let terminal_index =
    build_terminal_index terminal_rules
  in

  let chart = Hashtbl.create 1000 in

  (* Initialize lexical entries.
     Important for the browser version:
     Do NOT scan every lexical rule for every input word.  With a large
     generated lexicon, that can hit the browser/JavaScript call-stack limit.
     Instead, look up only the categories for the current word. *)
  for i = 0 to n - 1 do
    let word = input_arr.(i) in
    let categories =
      match Hashtbl.find_opt terminal_index word with
      | Some cats -> cats
      | None -> []
    in
...
let normalize_words words =
  words
  |> List.map String.lowercase_ascii
  |> List.sort_uniq String.compare

let remove_words banned words =
  let banned = normalize_words banned in
  List.filter
    (fun word -> not (List.mem (String.lowercase_ascii word) banned))
    words

let determiners =
  normalize_words Lexicon_generated.determiners

let wh_determiners =
  normalize_words Lexicon_generated.wh_determiners

let pronouns =
  normalize_words Lexicon_generated.pronouns

let adjectives =
  normalize_words Lexicon_generated.adjectives

let adverbs =
  normalize_words Lexicon_generated.adverbs

let verbs =
  normalize_words Lexicon_generated.verbs

let participles =
  normalize_words Lexicon_generated.participles

let gerunds =
  normalize_words Lexicon_generated.gerunds

let auxiliaries =
  normalize_words Lexicon_generated.auxiliaries

let modals =
  normalize_words Lexicon_generated.modals

let tense_words =
  normalize_words Lexicon_generated.tense_words

let prepositions =
  normalize_words Lexicon_generated.prepositions

let complementizers =
  normalize_words Lexicon_generated.complementizers

let question_auxiliaries =
  normalize_words Lexicon_generated.question_auxiliaries

let wh_question_auxiliaries =
  normalize_words Lexicon_generated.wh_question_auxiliaries

let wh_prepositions =
  normalize_words Lexicon_generated.wh_prepositions

let coordinators =
  normalize_words Lexicon_generated.coordinators

let trace_words =
  normalize_words Lexicon_generated.trace_words

let numbers =
  normalize_words Lexicon_generated.numbers

let units =
  normalize_words Lexicon_generated.units

let currencies =
  normalize_words Lexicon_generated.currencies

(* Closed-class words should not also be treated as ordinary nouns.
   This prevents fake ambiguities like:
     you = Pron
     you = N
*)
let closed_class_words =
  determiners
  @ wh_determiners
  @ pronouns
  @ auxiliaries
...
let nouns =
  Lexicon_generated.nouns
  |> normalize_words
  |> remove_words closed_class_words
...
let english_lexical_rules =
  ...
  @ [TRule (P_Of, "of");
     ...
     (* Optional explicit feature/trace tokens. *)
     TRule (TraceDP, "tdp");
     TRule (TraceWH, "twh");
     TRule (TraceT, "tt");
     TRule (T_Pres, "pres");
...
(* -------------------------------------------------------------------------- *)
(* Browser-safe English parser helpers                                         *)
(* -------------------------------------------------------------------------- *)

(*
   The full generated lexicon can contain thousands of TRule entries.  That is
   okay for native OCaml, but in the browser it is easy to hit JavaScript's
   call-stack limit if every parse scans every lexical rule.

   For the web terminal, we build a tiny per-sentence CFG:
   - keep the full English syntax rules,
   - keep only the lexical rules needed by the words in the input sentence.

   This preserves the same grammar behavior for the sentence being parsed, but
   avoids carrying the entire generated lexicon through the chart parser.
*)

let add_english_lexicon_entry table category word =
  let old =
    match Hashtbl.find_opt table word with
    | Some cats -> cats
    | None -> []
  in
  if List.mem category old then
    ()
  else
    Hashtbl.replace table word (category :: old)

let add_english_lexicon_list table category word_list =
  List.iter
    (fun word -> add_english_lexicon_entry table category word)
    word_list

let english_lexicon_index =
  let table = Hashtbl.create 8192 in

  add_english_lexicon_list table D determiners;
  add_english_lexicon_list table WhD wh_determiners;
  add_english_lexicon_list table Pron pronouns;
  add_english_lexicon_list table N nouns;
  add_english_lexicon_list table A adjectives;
  add_english_lexicon_list table Adv adverbs;
  add_english_lexicon_list table V verbs;
  add_english_lexicon_list table Participle participles;
  add_english_lexicon_list table Gerund gerunds;
  add_english_lexicon_list table Aux auxiliaries;
  add_english_lexicon_list table Modal modals;
  add_english_lexicon_list table T tense_words;
  add_english_lexicon_list table P prepositions;
  add_english_lexicon_list table C complementizers;
  add_english_lexicon_list table C_Q question_auxiliaries;
  add_english_lexicon_list table C_Q_WH wh_question_auxiliaries;
  add_english_lexicon_list table WhPP wh_prepositions;
  add_english_lexicon_list table Coord coordinators;
  add_english_lexicon_list table Num numbers;
  add_english_lexicon_list table Unit units;
  add_english_lexicon_list table Currency currencies;

  (* Explicit lexical rules that are not just category-list expansions. *)
  add_english_lexicon_entry table P_Of "of";
  add_english_lexicon_entry table TraceDP "tDP";
  add_english_lexicon_entry table TraceWH "tWH";
  add_english_lexicon_entry table TraceT "tT";
  add_english_lexicon_entry table T_Pres "pres";
  add_english_lexicon_entry table T_Past "past";
  add_english_lexicon_entry table C_WH "whC";

  table

let lexical_categories_for_word word =
  match Hashtbl.find_opt english_lexicon_index word with
  | Some cats -> List.rev cats
  | None -> []

let categories_for_word word =
  List.map show_cat (lexical_categories_for_word word)

let rec unique_input_words_helper seen xs =
  match xs with
  | [] -> List.rev seen
  | x :: rest ->
      if List.mem x seen then
        unique_input_words_helper seen rest
      else
        unique_input_words_helper (x :: seen) rest

let unique_input_words xs =
  unique_input_words_helper [] xs

let english_lexical_rules_for_input input_words =
  list_concat_map
    (fun word ->
       List.map
         (fun cat -> TRule (cat, word))
         (lexical_categories_for_word word))
    (unique_input_words input_words)

let english_cfg_for_input input_words =
  let lexical_rules =
    english_lexical_rules_for_input input_words
  in
  (
    all_cats,
    input_words,
    S,
    english_syntax_rules @ lexical_rules
  )

let missing_words input_words =
  List.filter
    (fun word -> lexical_categories_for_word word = [])
    (unique_input_words input_words)

let parse_english_for_web sentence =
  let sentence = String.lowercase_ascii sentence in
  let input_words = words sentence in

  if List.length input_words > 14 then
    "Parsing: " ^ sentence ^ "\n\n"
    ^ "Sentence too long for browser demo.\n"
    ^ "Try 14 words or fewer."
  else
    try
      let missing = missing_words input_words in
      ...
      if missing <> [] then
        ...
        let cfg = english_cfg_for_input input_words in
        let trees = parse_trees_chart cfg input_words in
        let shown = take_n 10 trees in
        ...
        | _ ->
            let body =
              ...
              |> List.mapi
                   (fun i tree ->
                     "Parse " ^ string_of_int (i + 1) ^ ":\n"
                     ^ tree_to_brackets show_cat (fun s -> s) tree)
              |> String.concat "\n\n"
            in

            "Parsing: " ^ sentence ^ "\n\n"
            ^ "Showing up to 10 parses.\n\n"
            ^ body
    ...
    | exn ->
        "Parsing: " ^ sentence ^ "\n\n"
        ^ "Parser error: " ^ Printexc.to_string exn

let lexical_report_for_web sentence =
  let sentence = String.lowercase_ascii sentence in
  let toks = words sentence in
  toks
  |> List.map
       (fun w ->
          match categories_for_word w with
          | [] -> w ^ ": MISSING"
          | cats -> w ^ ": " ^ String.concat ", " cats)
  |> String.concat "\n"

  let categories_for_word word =
  let word = String.lowercase_ascii word in
  let (_, _, _, rules) = english_xbar_cfg in
  rules
  |> List.filter_map
       (function
        | TRule (cat, w) when String.lowercase_ascii w = word ->
            Some (show_cat cat)
        | _ -> None)
  |> List.sort_uniq String.compare
